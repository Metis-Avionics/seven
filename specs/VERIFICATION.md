...
| QUIC sockets (S06, quinn 0.11 + pinned tokio) | `quic::loopback_send_recv_preserves_message` (ephemeral loopback, throwaway self-signed cert, 26 ms) | seven-mql | `QuinnEndpoint` + `send_message`/`recv_message` over bidirectional streams; TLS identity is caller-supplied (Seven never mints deployment credentials); remaining: deployment config, congestion tuning, stream reuse |
...
- [x] Property/invariant tests cover the core math contracts — 66 tests.
