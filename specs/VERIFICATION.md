...
| QUIC sockets (S06, quinn 0.11 + pinned tokio) | `quic::loopback_send_recv_preserves_message` (ephemeral loopback, throwaway self-signed cert, 26 ms) + `quic::loopback_batch_preserves_order` (5-message batch + empty batch over one stream) | seven-mql | `QuinnEndpoint` + `send_message`/`recv_message` over fresh streams, `send_messages`/`recv_messages` count-prefixed batches; TLS identity is caller-supplied (Seven never mints deployment credentials); remaining: deployment config, congestion tuning |
...
- [x] Property/invariant tests cover the core math contracts — 67 tests.
